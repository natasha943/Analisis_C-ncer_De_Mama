...
# Análisis Predictivo del Dataset de Cáncer de Mama:

Un Enfoque Integral con PCA, KNN y Redes Neuronales
 <center>
...
* DESCRIPCIÓN GENERAL:
  - El dataset contiene mediciones morfológicas obtenidas de imágenes
digitalizadas de tejido mamario, utilizadas para diagnosticar
tumores como Benignos (1) o Malignos (0).
  - Permite diagnosticar cáncer de mama de manera NO invasiva.
  - Contiene métricas obtenidas por computadora, evitando sesgos humanos.
  - Presenta características altamente informativas que facilitan
     el entrenamiento de modelos con precisión superior al 95%.
  - Es un estándar internacional para evaluar algoritmos de clasificación.


* ESTRUCTURA DEL DATASET:
   - Total de muestras: 569 pacientes
   - Total de variables: 30 características numéricas + 1 variable objetivo
* CONSIDERACIONES:
   - El dataset presenta outliers naturales debido a la variabilidad
     clínica de los tumores.
   - Es altamente linealmente separable, lo que permite buen
     rendimiento en modelos como KNN, PCA y redes neuronales.
...
El PCA es altamente efectivo en este dataset. Con solo 10 componentes se conserva la mayor parte de la información relevante (95%), reduciendo significativamente la dimensionalidad frente a las 30 variables originales.

Esto permite mejorar la eficiencia computacional y disminuir redundancia entre características altamente correlacionadas.
...
0% de pérdida de precisión al aplicar PCA (ambos modelos obtienen 96.49%).

La red neuronal clasifica correctamente 96 de cada 100 pacientes.

PCA reduce la dimensionalidad, pero mantiene el mismo rendimiento del modelo original.

Las dos versiones del modelo muestran idéntico desempeño, lo que indica que la información esencial del dataset se conserva incluso tras la reducción de características.
...
La preparación para regresión revela que mean area está altamente correlacionada con medidas de tamaño del tumor, y que PCA permite reducir de 29 a 10 dimensiones conservando el 95% de la información esencial para modelar.
...
El análisis visual de los cuatro gráficos de dispersión revela que tanto el algoritmo de vecinos más cercanos (KNN) como las Redes Neuronales (NN) son altamente efectivos para la tarea de regresión de predecir el "mean area", demostrando una precisión excelente y una fuerte correlación lineal entre los valores reales y los predichos.

Es notable que la inclusión del Análisis de Componentes Principales (PCA), una técnica común para la reducción de dimensionalidad, tuvo un impacto mínimo en el rendimiento de ambos modelos.

Con esto las cuatro configuraciones evaluadas (KNN/NN con o sin PCA) son igualmente robustas y confiables para esta aplicación específica.
...
El mejor modelo para la regresión es KNN con PCA, logrando el error más bajo (MAE = 56.45) y el mayor poder explicativo (R² = 0.9545), mientras que el peor desempeño lo presenta la red neuronal con PCA.
...
Los resultados de clasificación muestran que el modelo KNN sin PCA obtuvo el mejor desempeño con un Accuracy de 0.982456 (98.25%), clasificando correctamente casi 98 de cada 100 casos, lo que confirma que la estructura original del dataset favorece este algoritmo basado en distancias.

 Al aplicar PCA, el rendimiento de KNN disminuye a 0.956140 (95.61%), evidenciando una pérdida del 2.64%, ya que la reducción de dimensionalidad altera la geometría de los datos esenciales para este método.

 La red neuronal logró un Accuracy de 0.964912 (96.49%) tanto sin como con PCA, mostrando estabilidad y robustez al trabajar en un espacio reducido sin pérdida de información significativa.

 En síntesis, KNN sin PCA es el modelo más preciso, mientras que PCA afecta negativamente al KNN pero no modifica el rendimiento de la red neuronal, que se mantiene constante alrededor del 96.5%.
...
* Las predicciones realizadas con ejemplos reales muestran un desempeño perfecto de los modelos: el caso benigno con valores moderados de tamaño (por ejemplo, mean radius = 11.13 y mean area = 381.1) fue clasificado correctamente como clase 1 (benigno) tanto por KNN como por la red neuronal, con una probabilidad de 1.0000 para benigno según la NN.

* El caso maligno con medidas significativamente más grandes y agresivas (como mean radius = 19.55 y worst area = 1926.0) fue clasificado correctamente como clase 0 (maligno), con una probabilidad de 1.0000 para maligno en la red neuronal.

* En la sección de ejemplos sintéticos, los valores construidos para un caso benigno (por ejemplo, mean concave points = 0.03 y worst perimeter = 80) y para un caso maligno (con valores aumentados como mean concave points = 0.15 y worst perimeter = 150) permitieron validar la consistencia del modelo.

* Esto demuestra que tanto KNN como la red neuronal no solo clasifican bien casos reales, sino que también responden de forma estable ante variaciones sintéticas diseñadas para imitar comportamientos clínicos reales.

<h1> Conclusiones

El análisis integral del dataset Wisconsin Breast Cancer demuestra que este conjunto de datos posee una alta capacidad discriminativa entre tumores benignos y malignos, lo que permitió desarrollar modelos predictivos con excelente desempeño.

Tras la preparación y exploración inicial del dataset —incluyendo validación, estadística descriptiva, correlaciones, outliers y PCA— se identificó que las variables relacionadas con el tamaño y la forma del núcleo celular (como mean radius, mean perimeter y mean area) son determinantes para ambas tareas, clasificación y regresión.

En los modelos de clasificación, KNN sin PCA alcanzó la mayor precisión con 98.25%, superando a KNN con PCA y mostrando que la estructura original del espacio de características favorece este algoritmo. Las redes neuronales mantuvieron un desempeño estable alrededor del 96.5%, confirmando su robustez incluso en espacios reducidos.

En la tarea de regresión, el mejor modelo fue KNN con PCA, con un R² de 0.9545 y los errores más bajos, evidenciando que la reducción de dimensionalidad mejora la predicción del mean area. Finalmente, las predicciones con ejemplos reales y sintéticos mostraron coherencia total, clasificando correctamente tanto casos benignos como malignos con probabilidades extremas de 1.0000, lo que valida la solidez y confiabilidad de los modelos implementados.

 En conjunto, el proyecto demuestra que técnicas clásicas como KNN y métodos modernos como redes neuronales pueden lograr resultados altamente precisos en diagnósticos asistidos por inteligencia artificial, reforzando el potencial de estas herramientas en aplicaciones médicas.
